...
- The Livepatch admin tool [setup](./setup-administration-tool) against the existing server.

When deploying an instance of the Livepatch server (see our existing [guides](./index)) you will normally use an Ubuntu Pro token to configure the on-premise server to sync with Canonical's hosted Livepatch server.
...
See our [reference doc](/livepatch_on_prem/reference/patch-management.md) to better understand how tiers work and how patches are synced.
