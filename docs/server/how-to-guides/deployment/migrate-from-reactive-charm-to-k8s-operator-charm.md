...
```{note}
If the migrated configuration includes `auth.sso.*` keys, note that Ubuntu SSO authentication has been removed. Remove those keys and configure Basic Auth (`auth.basic.*`) instead.
```
