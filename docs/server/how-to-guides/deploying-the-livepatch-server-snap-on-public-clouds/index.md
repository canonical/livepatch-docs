...
write_files:
  - path: /etc/client/livepatch.env
    permissions: "0600"
    owner: root:root
    content: |
      export URLTEMPLATE="http://<address>/v1/patches/{filename}"
  - path: /etc/client/database_url
    permissions: "0600"
    owner: root:root
  - path: /etc/client/pro_token
    permissions: "0600"
    owner: root:root
  - path: /etc/client/livepatch_admin_user
    permissions: "0600"
    owner: root:root
    # Storage option
       ...
...
runcmd:
  ...
  - |
    ...
    canonical-livepatch-server.schema-tool "$(cat /etc/client/database_url)"      
    
    snap set canonical-livepatch-server \
    token="$(cat /etc/client/pro_token)" \
    lp.database.connection-string="$(cat /etc/client/database_url)" \
    lp.server.url-template=$URLTEMPLATE \
    lp.auth.basic.enabled=true \
    lp.patch-sync.enabled=true \
    lp.patch-sync.interval=12h \
    lp.auth.basic.users="$(cat /etc/client/livepatch_admin_user)" \
...
This cloud-init module expects to find required parameters (secrets, user strings, database DNS) in root-only files located at `/etc/client/` during boot time. Specifics on getting required secrets are unique to the cloud environment. We recommend using the cloud’s vault provider to securely access the secrets and write them to the required locations. The files are deleted during the last step of the cloud-init setup.
...
snap set canonical-livepatch-server lp.auth.basic.users="$(cat /etc/client/livepatch_admin_user)"
```

Where the user and hashed password in the form of _**username:hashedpassword**_ are fetched from a secure vault and written to a root-only file `/etc/client/livepatch_admin_user`.
