...
- Configure the relevant S3 [config options](/server/reference/configuration.md)

Once this is configured, livepatch will store and retrieve patch files from the S3 bucket.

A further improvement is to configure livepatch on-prem to serve patches from the S3 bucket directly. For that public http access needs to be allowed to that bucket. Set your server's [URL template config](/server/reference/configuration.md) to something resembling:
