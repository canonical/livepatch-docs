...
runcmd:
  - |
    ...
    # fetch pro token from a vault and redirect into /etc/client/pro_token
  - |
    set -e
    snap install canonical-livepatch
    pro attach $(cat /etc/client/pro_token)
...
To configure the Livepatch client through the cloud-init module, replace each template value in the write_files section for `/etc/client/livepatch.env`, such as `__LOG_LEVEL__`, with a valid configuration based on the [available configuration options](/client/reference/configuration-options.md).

> Note: To enable Livepatch, you need to attach the instance to Ubuntu Pro. Do not write the Pro token into the cloud-init module. Instead, store the pro token in a secrets vault, and access the pro token from the vault, writing the token into a root-owned file at`/etc/client/pro_token`. The cloud-init module reads from this file to attach to Ubuntu Pro. The file is deleted at the last step of the cloud-init setup process.
...
runcmd:
  - |
    set -e
    snap wait system seed.loaded
    # fetch token from secrets vault and redirect to /etc/client/authToken
  - |
    set -e
    snap install canonical-livepatch
    . /etc/client/livepatch.env
    canonical-livepatch config \
      ...
      no-proxy="$NO_PROXY" \
      tls-patch-download="$TLS_PATCH_DOWNLOAD"
    canonical-livepatch enable $(cat /etc/client/authToken)
  - |
    rm /etc/client/*
final_message: The system is up, up to date, and Livepatch client is active after $UPTIME seconds
```
...
> Note: To enable Livepatch with an on-prem Livepatch server, you need to attach the instance with an auth token. Do not write the auth token into the cloud-init module. Instead, store the auth token in a secrets vault, and redirect to `/etc/client/authToken`.
> 
