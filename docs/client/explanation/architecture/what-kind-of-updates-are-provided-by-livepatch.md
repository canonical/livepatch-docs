...
Live kernel patches are developed and released based on kernel SRU releases, and contain a subset of the CVEs fixed by that kernel SRU. Live kernel patches are cumulative, so when a new live kernel patch is released, it contains both the new CVE fixes, as well as all of the CVE fixes from the previous release of that live kernel patch.
